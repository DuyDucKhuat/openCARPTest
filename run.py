...
.. figure:: /images/01_01_MyocyteEP_Setup.png
   ...
   stimulus current to initiate action potentials at a pacing cycle length of 500 ms.
...
.. code-block:: bash
  ...
  --EP {tenTusscherPanfilov, DrouhardRoberge, OHara}        pick human EP model (default is tenTusscherPanfilov)
  ...
  --overlay             overlays the action potential or specified variables of several
                        experiments in the same plot

  --vis_var VIS_VAR     specified the variables that you want to visualize (default is V)
...
In the PZ various currents are downregulated.
...
and a reduction of 70% and 80% in potassium currents :math:`I_{\mathrm {Kr}}` and :math:`I_{\mathrm {K1}}`, respectively. 
...
The reference traces are passed in and you can compare the experiment output.
...
def run(args, job):
    ...
    if args.EP == 'DrouhardRoberge':
        pass
    elif args.EP in ['tenTusscherPanfilov', 'OHara']:
        cmd += ['--stim-species', 'Ki']
    # -------------------------------------------------------------------------
